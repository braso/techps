...
function index() {
	...
	$c[] = combo('Situação:','busca_situacao',$_POST[busca_situacao],2,array('Todos','Verificados','Não conformidade'));
	...
	if($_POST[busca_data] && $_POST[busca_empresa]){
		...
		while($aMotorista = carrega_array($sqlMotorista)){
			...
			for ($i = 1; $i <= $daysInMonth; $i++) {
				...
				if($_POST[busca_situacao] == 'Todos'){
					//NAO FAZ NADA
				}elseif($_POST[busca_situacao] == 'Não conformidade'){
					if (
