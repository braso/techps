...
function cadastra_motorista() {
	...
	$campos = array(
		enti_tx_nome, enti_tx_nascimento, enti_tx_cpf, enti_tx_rg, enti_tx_civil, enti_tx_sexo, enti_tx_endereco, enti_tx_numero, enti_tx_complemento,
	...
	if (in_array($file_type, $allowed) && $_FILES[foto][name] != '') {
		...
		if ($arq) {
			...
			atualizar('user', array('user_tx_foto'), array($arq), $idUser);
		}
	}
...
function layout_motorista() {
	...
	$c[] = combo('Estado Civil', 'civil', $a_mod[enti_tx_civil], 2, array('', 'Casado(a)', 'Solteiro(a)', 'Divorciado(a)', 'Viúvo(a)'));
