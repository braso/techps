		...
		<section>
			<table id="t01">
				...
				<?
					...
					$sql = query("SELECT * FROM entidade WHERE enti_tx_status != 'inativo' AND enti_tx_tipo = 'Cliente' $extra ORDER BY enti_tx_nome ASC");

					while($a = carrega_array($sql)){

						if ($a[enti_nb_cidade]>0) {
							$a_cidade = carregar('cidade',$a[enti_nb_cidade]);
						} else {
							$a_cidade[cida_tx_nome] = '';
						}
						...
						<tr>
							...
							<td style="text-align: center;"><?=$a[enti_tx_cel]?></td>
							<td style="text-align: center;"><?=$a[enti_tx_email]?></td> 						
							<td style="text-align: center;"><?=$a_cidade[cida_tx_nome]?></td> 						
						</tr>
						<?
					}
				?>
