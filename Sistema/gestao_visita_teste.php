...
function cadastra_visita(){
	global $a_mod;

	$campos = array(visi_nb_entidade,visi_nb_imovel,visi_nb_corretor,visi_tx_descricao,visi_nb_cidade,visi_tx_bairro,visi_tx_finalidade,visi_tx_tipo,
		visi_tx_situacao,visi_tx_dataAgenda,visi_tx_horaAgenda,visi_tx_dataCadastro,visi_nb_userCadastro,visi_nb_construtor,visi_tx_status);
	$valores = array($_POST[cliente],$_POST[imovel],$_POST[corretor],$_POST[descricao],$_POST[cidade],$_POST[bairro],$_POST[finalidade],$_POST[tipo],
		'pendente',$_POST[data],$_POST[hora],date("Y-m-d"),$_SESSION[user_nb_id],$_POST[construtor],'ativo');
...
function layout_visita(){
	...
	$c[]=campo_data('Data','data','',2);
	$c[]=campo_hora('Hora','hora','',1);
	$c[]=combo_net('Cliente','cliente',$a_mod[visi_nb_entidade],3,'entidade','',"AND enti_tx_tipo = 'Cliente' $extra_cliente");
	$c[]=combo_net('Corretor','corretor',$a_mod[visi_nb_corretor],3,'entidade','',"AND (enti_tx_tipo = 'Corretor' OR enti_tx_tipo = 'Administrador') ");
	$c[]=combo_net('Imóvel','imovel',$a_mod[visi_nb_imovel],3,'imovel');
	$c[]=ckeditor('Descrição Imóvel','descricao',$a_mod[visi_tx_descricao],12);
	$c[]=combo_net('Cidade','cidade',$a_mod[visi_nb_cidade],3,'cidade');
	$c[]=campo('Bairro','bairro',$a_mod[visi_tx_imovel],3);
	$c[]=combo('Finalidade','finalidade',$a_mod[visi_tx_finalidade],2,array("","Venda","Aluguel"));
	// $c[]=combo('Tipo','tipo',$a_mod[visi_tx_tipo],2,array("","Casa","Prédio ou Casa Comercial","Apartamento","Terreno"));
	$c[]=combo_bd('!Tipo','tipo',$a_mod[visi_nb_tipoimovel],2,'tipoimovel');
	$c[]=combo_bd('!Construtor','construtor',$a_mod[visi_nb_construtor],2,'construtor');
			
	$b[]=botao('Gravar','cadastra_visita','id',$_POST[id]);
...
function cadastra_remarcacao(){
	...
	$campos = array(visi_nb_entidade,visi_nb_imovel,visi_nb_corretor,
		visi_tx_descricao,visi_nb_cidade,visi_tx_bairro,
		visi_tx_finalidade,visi_tx_tipo,visi_tx_situacao,visi_tx_dataAgenda,visi_tx_horaAgenda,
		visi_tx_dataCadastro,visi_nb_userCadastro,visi_nb_construtor,visi_tx_status);
	
	$valores = array($a_visita[visi_nb_entidade],$a_visita[visi_nb_imovel],$a_visita[visi_nb_corretor],
		$a_visita[visi_tx_descricao],$a_visita[visi_nb_cidade],$a_visita[visi_tx_bairro],
		$a_visita[visi_tx_finalidade],$a_visita[visi_tx_tipo],'pendente',$_POST[data],$_POST[hora],
		date("Y-m-d"),$_SESSION[user_nb_id],$a_visita[visi_nb_construtor],'ativo');
...
function layout_remarcar(){

	if ( intval($_POST[id])==0 ) {
		set_status('ERRO: Falha ao carregar os dados da visita!');
		index();
		exit;
	}
	...
	$c[]=campo_data('Nova Data','data','',2);
	$c[]=campo_hora('Nova Hora','hora','',1);
...
function index(){
	...
	$val = array('visi_nb_id','enti_tx_nome','carrega_corretor(visi_nb_corretor)','data(visi_tx_dataAgenda)','visi_tx_horaAgenda','visi_tx_situacao',
		...
		'icone_modificar(visi_nb_id,layout_remarcar,,,,glyphicon glyphicon-refresh)',
