...
function adiciona_item_pagamento($id_ordem=0,$id_forma=0,$valor=0,$parcelas=0){
	...
	if ( $id_ordem==0 ) {
		...
		$sql_venda = query("SELECT enti_tx_clientePadrao FROM ordem,entidade WHERE orde_nb_entidade=enti_nb_id AND orde_nb_id='$id_ordem' LIMIT 1");
