...
function adiciona_produto(){
	...
	// CARREGA O PRODUTO PARA VERIFICAR SE O ESTOQUE ESTÁ OU VAI FICAR NEGATIVO
	$a_prod = carregar('produto',$id_produto);
	if ( strtolower($a_prod[prod_tx_servico])!='sim' && ($a_prod[prod_tx_qtde]-$quantidade)<0 ) {
		$pedir_senha = 'sim';// SEMPRE PEDE A SENHA POR PADRÃO

		if ( trim($_POST[senha_solicitada])=='sim' ) {
			// SE EXIBIU O MODAL DA SENHA, VALIDA A PARTE DA SENHA GERENCIAL
			$_POST[senha_gerencial] = trim($_POST[senha_gerencial]);

			if ( $_POST[senha_gerencial]=='' ) {
				// SE NÃO INFORMOU A SENHA
				?><script type="text/javascript">
					alert('ATENÇÃO: Informe uma senha gerencial!');
				</script><?php
			} else {
				// SE INFORMOU A SENHA, VERIFICA SE É VÁLIDA
				$sql_senha = query("SELECT * FROM user WHERE user_tx_status='ativo' AND (user_tx_nivel='Administrador' OR user_tx_nivel='Gerente') AND user_tx_senha='".md5($_POST[senha_gerencial])."' LIMIT 1");
				if ( num_linhas($sql_senha)>0 ) {
					// SE ACERTOU A SENHA
					$pedir_senha = '';
				} else {
					// SE ERRAR A SENHA
					?><script type="text/javascript">
						alert('ATENÇÃO: Senha gerencial inválida!');
					</script><?php
				}
			}
		}

		// VERIFICA SE PRECISA PEDIR A SENHA
		if ( $pedir_senha=='sim' ) {			
			// EVITA QUE A MESMA VARIÁVEL POST SEJA CRIADA MAIS DE UMA VEZ
			unset($_POST[senha_solicitada],$_POST[senha_gerencial]);

			?><script type="text/javascript">
				$("#dados_extras_modal_senha").val("<?=http_build_query($_POST)?>");
				$("#cp_senha_gerencial").val("");
				$("#ModalSenhaLabel").html("<b>ATENÇÃO: PRODUTO COM ESTOQUE NEGATIVO!</b>");
				$("#modalSenha").modal("show");
			</script><?php
			exit;

		} else {
			// SE NÃO PRECISAR SOLICITA A SENHA, ESCONDE O MODAL DA SENHA
			?><script type="text/javascript">
	...
	// SE ESTIVER TUDO OK, ATUALIZA A LISTAGEM DA VENDA
	?><script type="text/javascript">
...
function adiciona_pagamento(){
	...
	$_POST[id_venda] = intval($_POST[id_venda]);
	$_POST[id_forma] = intval($_POST[id_forma]);
	$pedir_senha = '';
	...
	} else {// SE NÃO EXIBIU O MODAL DA SENHA, VERIFICA SE PRECISA PEDIR A SENHA

		// AQUI VERIFICA SE O CLIENTE NÃO FOI IDENTIFICADO E SE É UM RECEBIMENTO EM CARTEIRA
		$sql_venda = query("SELECT * FROM ordem,entidade WHERE orde_nb_entidade=enti_nb_id AND orde_nb_id='$_POST[id_venda]' LIMIT 1");
		$a_cliente = $sql_venda->fetch_assoc();
		if ( strtolower($a_cliente[enti_tx_clientePadrao])!='sim' ) {
			// SE FOR UM CLIENTE IDENTIFICADO, VERIFICA SE É UM PAGAMENTO EM CARTEIRE E CHECA O LIMITE DE CRÉDITO DO MESMO
			
			$a_forma = carregar('forma',$_POST[id_forma]);
			// SE FOR EM CARTEIRA, VERIFICA SE O LIMITE DE CRÉDITO DO CLIENTE FOI ATINGIDO
			if ( $a_cliente[enti_tx_limiteCredito]>0 && strtolower($a_forma[form_tx_carteira])=='sim' ) {
				// SOMA O QUE JÁ FOI PAGO EM CARTEIRA NA VENDA
				$sql_soma_carteira = query("SELECT SUM(paga_tx_valor) AS total FROM pagamento,forma WHERE paga_nb_forma=form_nb_id AND paga_nb_ordem='$_POST[id_venda]' AND paga_tx_status='ativo' AND form_tx_carteira='sim' ");
				$a_soma_carteira   = carrega_array($sql_soma_carteira);
				// VERIFICA SE O PAGAMENTO ATUAL ESTOURA O LIMITE DO CLIENTE
				if ( ($a_soma_carteira[total]+valor($_POST[valor_pagamento],1)) > $a_cliente[enti_tx_limiteCredito] ) {
					$pedir_senha = 'sim';// PEDE A SENHA PARA LIBERAR O PAGAMENTO
				}
			}
		}
	...
	if ( $pedir_senha=='sim' ) {
		...
		?><script type="text/javascript">
			$("#dados_extras_modal_senha").val("<?=http_build_query($_POST)?>");
			$("#cp_senha_gerencial").val("");
			$("#ModalSenhaLabel").html("<b>ATENÇÃO: LIMITE DE CRÉDITO DO CLIENTE ATINGIDO!</b>");
			$("#modalSenha").modal("show");
		</script><?php
		exit;
