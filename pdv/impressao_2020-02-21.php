	...
	$a_user  = carregar('user',$a_caixa[caix_nb_user]);
		...
		<section>
			...
			<table id="t01">
				...
				// CRIA UM ARRAY PARA TOTALIZAR OS RECEBIMENTOS POR FORMA DE PAGAMENTO
				$a_resumo_forma = array();
				...
				while($row = carrega_array($sql_recebimentos)){
					...
					$a_enti  = carregar('entidade',$row[orde_nb_entidade]);

					?><tr>
						<td class="text-center"><?=$j?></td>
						<td class="text-center"><?=$row['orde_nb_id']?></td>
						<td><?=$a_enti['enti_tx_nome']?></td>
					...
					// TOTALIZA OS RECEBIMENTOS POR FORMA
					$a_resumo_forma[$row[fobo_nb_forma]] += $row['fobo_tx_valor'];
				}
		...
		<hr>
		<section>
			<p><b>RESUMO DE RECEBIMENTOS:</b></p>
			<table id="t02" style="width: 50%">
				...
				foreach ($a_resumo_forma as $id_forma => $valor) {
				...
				?><tr>
					<td><b>TOTAL:</b></td>
					<td><b><?=valor($total_recebimento,1)?><b></td>
				</tr><?php
				?>
			</table>
		</section>
	...
	$a_entidade = carregar('entidade',$a_ordem[orde_nb_entidade]);
		...
		<section>
			...
			<table id="t01">
				...
				if ( $a_ordem['orde_tx_valorFrete']>0 ) {
					...
					?><tr>
						...
						<td class="text-center"><?=valor($a_ordem['orde_tx_valorFrete'],1)?></td>
						<td class="text-center">&nbsp;</td>
						<td class="text-center"><?=valor($a_ordem['orde_tx_valorFrete'],1)?></td>
					</tr><?php
					// SOMA AO VALOR TOTAL DA VENDA
					$valor_total_venda += $a_ordem['orde_tx_valorFrete'];
				}
