...
function layout_relatorio(){
	...
	$extra .= " AND bole_tx_status = 'Encerrado'";// APENAS BOLETOS ENCERRADOS
	$extra .= " AND (plan_tx_retiradaDinheiro='' OR plan_tx_retiradaDinheiro IS NULL) ";// NÃO EXIBE O PLANO RETIRADA DE DINHEIRO DO CAIXA
	...
	$sql_receita = query("SELECT * FROM boleto,entidade,movimento,planoconta WHERE movi_nb_planoconta = plan_nb_id AND bole_nb_movimento = movi_nb_id AND movi_nb_entidade = enti_nb_id AND bole_tx_status != 'inativo' $extra_receita $extra ORDER BY bole_tx_data ASC, enti_tx_nome ASC");
	...
	$sql_despesas = query("SELECT * FROM boleto,entidade,movimento,planoconta WHERE movi_nb_planoconta = plan_nb_id AND bole_nb_movimento = movi_nb_id AND movi_nb_entidade = enti_nb_id AND bole_tx_status != 'inativo' $extra_despesa $extra ORDER BY bole_tx_data ASC, enti_tx_nome ASC");
