...
function exclui_venda(){
	...
	while ( $a_orpr = carrega_array($sql_orpr) ) {
		...
		// VERIFICA SE O ESTOQUE DO PRODUTO FOI ABATIDO PARA PODER RETORNAR A QUANTIDADE
		if ( strtolower($a_orpr[orpr_tx_estoqueAbatido])=='sim' ) {
			$a_produto = carregar('produto',$a_orpr[orpr_nb_produto]);

			$estoqueAnterior   = $a_produto[prod_tx_qtde];
			$estoqueQuantidade = $a_orpr[orpr_tx_quantidade];
			$estoqueNovo       = $estoqueAnterior+$estoqueQuantidade;
			// ATUALIZA O ESTOQUE DO PRODUTO
			atualizar('produto',array(prod_tx_qtde),array($estoqueNovo),$a_orpr[orpr_nb_produto]);

			// MARCA A ORPR PARA CONFIRMAR QUE A BAIXA DO ESTOQUE FOI FEITA
			atualizar('orpr',array(orpr_tx_estoqueAbatido),array('sim'),$a_orpr[orpr_nb_id]);
			...
			$valores = array($a_orpr[orpr_nb_produto],$estoqueNovo,
							$a_orpr[orpr_nb_id],$id_venda,$estoqueQuantidade,$estoqueAnterior,
							$_SESSION[user_nb_id],date('Y-m-d H:i:s'),'Cancelamento de Venda','ativo');
