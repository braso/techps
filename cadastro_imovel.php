...
function cadastra_imovel(){
	...
	if ( intval($_POST[tipo])==0 ) {
		// ADICIONA O TIPO INDEFINIDO CASO NÃO SELECIONE NADA
		$_POST[tipo]=10;
	}


	$campos = array(imov_tx_nome,imov_tx_financiavel,imov_nb_tipoimovel,imov_tx_finalidade,imov_tx_endereco,imov_tx_bairro,
		imov_tx_cep,imov_nb_cidade,imov_tx_obs,imov_tx_quadra,imov_tx_lote,imov_tx_areautil,imov_tx_areaconstruida,
		imov_tx_terreno,imov_tx_vlrvenda,imov_tx_vlrlocacao,imov_tx_vlriptu,imov_tx_vlrcondominio,
		imov_tx_comissao,imov_tx_dormitorios,imov_tx_suites,imov_tx_salas,imov_tx_banheiros,imov_tx_varanda,
		imov_tx_pavimentos,imov_tx_vagasgaragem,imov_tx_vagasvisitantes,imov_tx_deposito,imov_tx_elevador,imov_tx_piscina,
		imov_tx_churrasqueira,imov_tx_quadrapoliesportiva,imov_tx_situacao,imov_tx_dataCadastro,imov_nb_userCadastro,
		imov_nb_construtor,imov_tx_status);
	$valores = array($_POST[descricao],$_POST[financiavel],$_POST[tipo],$_POST[finalidade],$_POST[endereco],$_POST[bairro],
		$_POST[cep],$_POST[cidade],$_POST[obs],$_POST[quadra],$_POST[lote],valor($_POST[areautil]),valor($_POST[areaconstruida]),
		valor($_POST[terreno]),valor($_POST[vlrvenda]),valor($_POST[vlrlocacao]),valor($_POST[vlriptu]),valor($_POST[vlrcondominio]),
		valor($_POST[comissao]),$_POST[dormitorios],$_POST[suites],$_POST[salas],$_POST[banheiros],$_POST[varanda],
		$_POST[pavimentos],$_POST[vagasgaragem],$_POST[vagasvisitantes],$_POST[deposito],$_POST[elevador],$_POST[piscina],
		$_POST[churrasqueira],$_POST[quadrapoliesportiva],$_POST[situacao],date("Y-m-d"),$_SESSION[user_nb_id],
		$_POST[construtor],'ativo');
...
function layout_imovel(){
	...
	$c1[]=combo_bd('!Tipo','tipo',$a_mod[imov_nb_tipoimovel],2,'tipoimovel');
	...
	$c6[]=combo_bd('!Construtor','construtor',$a_mod[imov_nb_construtor],2,'construtor');
...
function index(){
	...
	if($_POST[busca_tipo])
		$extra .=" AND imov_nb_tipoimovel = '$_POST[busca_tipo]'";
	...
	if($_POST[busca_construtor])
		$extra .= " AND imov_nb_construtor = '$_POST[busca_construtor]' ";
	...
	$c[]=combo_bd('!Tipo','busca_tipo',$_POST[busca_tipo],2,'tipoimovel');
	...
	$c[]=combo_bd('!Construtor','busca_construtor',$_POST[busca_construtor],2,'construtor');
	...
	$sql = "SELECT * FROM imovel,tipoimovel WHERE imov_tx_status != 'inativo' AND imov_nb_tipoimovel = tipo_nb_id $extra";
	$cab = array('CÓDIGO','DESCRIÇÃO','TIPO','FINALIDADE','SITUAÇÃO','FINANCIÁVEL','','','');
	$val = array('imov_nb_id','imov_tx_nome','tipo_tx_nome','imov_tx_finalidade','imov_tx_situacao','imov_tx_financiavel','icone_modificar(imov_nb_id,layout_imagens,,,,glyphicon glyphicon-open-file)','icone_modificar(imov_nb_id,modifica_imovel)','icone_excluir(imov_nb_id,exclui_imovel)');		
